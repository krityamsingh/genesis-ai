...
#
# Single function that routes to any of the 4 AI providers:
#   gemma4  → HuggingFace InferenceClient (local/remote)
...
#   response = call_ai("Your prompt", provider="openai")
#   response = call_ai("Your prompt", provider="gemini")
# ============================================================
...
# ── Heal rotation: attempt number → provider ─────────────────
HEAL_ROTATION = {
    1: PROVIDER_OPENAI,
    2: PROVIDER_GROK,
    3: PROVIDER_GEMINI,
...
def call_ai(
    ...
    Returns:
        Response text as a string. Never raises — returns
        an error string prefixed with [CALLER ERROR] on failure.
    """
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "429" in err or "rate limit" in err or "quota" in err:
                ...
                print(f"[Caller] Rate limit on {provider} — waiting {wait}s...")
                time.sleep(wait)
            elif attempt < MAX_RETRIES - 1:
                print(f"[Caller] Retry {attempt+1}/{MAX_RETRIES} on {provider}: {e}")
...
def _call_gemma4(
    ...
    from core.gemma_engine import GEMMA4_MODELS

    token    = os.getenv("HF_TOKEN", "")
    model_key = os.getenv("GEMMA_MODEL", "default")
    model_id  = GEMMA4_MODELS.get(model_key, model_key)
...
def test_all_providers():
    ...
    Tests all 4 providers with a simple prompt.
    """
    print("=" * 50)
